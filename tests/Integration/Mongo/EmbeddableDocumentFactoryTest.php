...
use Zenstruck\Foundry\Tests\Fixture\Document\MongoEmbeddable;
...
final class EmbeddableDocumentFactoryTest extends EmbeddableFactoryTestCase
{
    ...
    public function embed_many(): void
    {
        $document = persist(WithEmbeddableDocument::class, [
            'embeddable' => $this->embeddableFactory()->with(['prop1' => 'value1']),
            'embeddables' => $this->embeddableFactory()->with(fn($i) => ['prop1' => 'value'.$i])->many(2),
        ]);
    ...
    {
        return factory(MongoEmbeddable::class); // @phpstan-ignore-line
    }
}
